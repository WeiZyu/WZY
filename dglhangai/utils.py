...
def setup(args):
    ...
    args['dataset'] = 'ACM'#'ACMRaw' if args['hetero'] else 'ACM'
...
def load_acm(remove_self_loop):
    ...
    # Fe = fea()
    # pd.DataFrame(Fe).to_excel(r'G:\图神经网络编程\有向异构\dglhan改\特征矩阵1.xlsx', index=None, header=None)
    #tff = np.eye(1734)
    tff = pd.read_excel(r'G:\图神经网络编程\有向异构\dglhan改\特征矩阵.xlsx', header=None)
    ...
    # a = pd.read_excel(r'C:\Users\lenovo\Desktop\SCI\第三篇SCI\自己整理的\全部的细胞因子\string_interactions.xlsx',header = None)
    ...
    # da1[1] = li1
    # da1[2] = a[2]
    # M_ = coo_matrix((da1.iloc[:, 2], (da1.iloc[:, 0], da1.iloc[:, 1])), shape=(len(dic), len(dic))).toarray()

    c = pd.read_excel(r'C:\Users\lenovo\Desktop\SCI\第三篇SCI\自己整理的\全部的细胞因子\CGC.xlsx',header = None)
    ...
    c11 = pd.read_excel(r'C:\Users\lenovo\Desktop\SCI\第三篇SCI\自己整理的\全部的细胞因子\CPC.xlsx',header = None)
    ...
    www1 = pd.read_excel(r'C:\Users\lenovo\Desktop\SCI\第三篇SCI\自己整理的\标签.xlsx', index=None)
